...
// Listener para manter a interface sincronizada com as senhas no Firebase
firebase.database().ref('senhas').on('value', (snapshot) => {
  const data = snapshot.val();

  if (!data) {
    // Se as senhas foram apagadas, resetar a interface.
  ...
  botoes.forEach(btn => {
    ...
    atualizarInterfaceSenhas(data);  // Idem: essa função deve atualizar os elementos visuais.
  }
});
