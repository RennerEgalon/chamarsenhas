...
function atualizarInterfaceComNovaChamada(registro) {
  // Atualiza o destaque dos botões se for uma senha
  if (registro.tipo === "Senha Normal" || registro.tipo === "Senha Preferencial") {
    ...
    const idColuna = isPreferencial 
      ? `coluna-preferencial-${registro.guiche.toLowerCase().replace(/\s/g, '')}`
      : `coluna-normal-${registro.guiche.toLowerCase().replace(/\s/g, '')}`;

    const coluna = document.getElementById(idColuna);
    if (coluna) {
      const botoes = Array.from(coluna.querySelectorAll('button'));
      botoes.forEach(btn => {
        if (btn.textContent === registro.senha) {
          const classeDestaque = isPreferencial ? 'botao-destacado-preferencial' : 'botao-destacado-normal';
          btn.classList.add(classeDestaque);
  ...
  // Se for chamada por nome, pode opcionalmente exibir no histórico visual
}
