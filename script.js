...
document.getElementById('botaoLimparSenhas').addEventListener('click', () => {
  firebase.database().ref('limparSenhas').set(Date.now()); // ou true
});

firebase.database().ref('limparSenhas').on('value', (snapshot) => {
  const sinal = snapshot.val();
  if (sinal) {
    // Limpa as interfaces locais
    limparInterfaceLocal();

    // Se quiser, também zera no Firebase:
    firebase.database().ref('contadorNormal').set(0);
    firebase.database().ref('contadorPreferencial').set(0);
    firebase.database().ref('maioresSenhasPorColuna').set({});
    firebase.database().ref('historicoChamadas').remove();
  }
});

function limparInterfaceLocal() {
  document.getElementById('ultimaSenhaNormal').textContent = 'Sem chamada';
  document.getElementById('ultimaSenhaPreferencial').textContent = 'Sem chamada';
  const historicoContainer = document.getElementById('historicoChamadas');
  if (historicoContainer) {
    historicoContainer.innerHTML = '<h3>Histórico de Chamadas</h3>';
  }
}
