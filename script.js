...
function configurarListenerSenhas() {
  const ref = firebase.database().ref('maioresSenhasPorColuna');
          ...
          botoes.forEach(btn => {
            ...
            if (match) {
              ...
              if (num <= limite) {
                ...
                btn.classList.remove(classeDestaque);
              }
            }
          });
        }
...
function configurarListenerUltimasSenhas() {
  const refNormal = firebase.database().ref('contadorNormal');
  const refPreferencial = firebase.database().ref('contadorPreferencial');

  refNormal.on('value', snapshot => {
    const valor = snapshot.exists() ? snapshot.val() : 0;
    atualizarUltimaSenhaNormal(`Senha ${valor}`);
  });

  refPreferencial.on('value', snapshot => {
    const valor = snapshot.exists() ? snapshot.val() : 0;
    atualizarUltimaSenhaPreferencial(`Senha ${valor}`);
  });
}


function salvarChamadaNoFirebase(registro) {
  const chamadaRef = firebase.database().ref('historicoChamadas').push();
  chamadaRef.set(registro);
}

function atualizarInterfaceComNovaChamada(registro) {
  if (registro.tipo === "Senha Normal" || registro.tipo === "Senha Preferencial") {
    const isPreferencial = registro.tipo === "Senha Preferencial";

    // Extrai a parte "Senha N" do registro
    const numeroSenha = registro.senha.match(/^Senha \d+/)[0];  

    // Classe de destaque
    const classeDestaque = isPreferencial ? 'botao-destacado-preferencial' : 'botao-destacado-normal';

    // Para cada guichê
    ['guiche1', 'guiche2'].forEach(guiche => {
      const idColuna = isPreferencial 
        ? `coluna-preferencial-${guiche}`
        : `coluna-normal-${guiche}`;
      ...
      if (coluna) {
        ...
        botoes.forEach(btn => {
          // Extrai a parte "Senha N" de cada botão
          const numeroBotao = btn.textContent.match(/^Senha \d+/);

          if (numeroBotao && numeroBotao[0] === numeroSenha) {
            btn.classList.add(classeDestaque);
            btn.scrollIntoView({ behavior: 'smooth', block: 'center' });
          } else {
            // Remove destaque de outros botões
            btn.classList.remove('botao-destacado-preferencial');
            btn.classList.remove('botao-destacado-normal');
          }
        });
      }
    });

    // Atualiza a exibição da última senha chamada
    if (isPreferencial) {
      atualizarUltimaSenhaPreferencial(registro.senha);
    } else {
      atualizarUltimaSenhaNormal(registro.senha);
    }
  }
}
...
salvarChamadaNoFirebase(registro);
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (confirmar) {
   ...
   configurarListenerSenhas();
   configurarListenerUltimasSenhas();
});
...
function escutarAlteracoesHistorico() {
  firebase.database().ref('historicoChamadas').on('child_added', snapshot => {
    const registro = snapshot.val();

    // Evitar duplicatas (opcional)
    if (!historicoChamadas.some(r => JSON.stringify(r) === JSON.stringify(registro))) {
      historicoChamadas.push(registro);
      atualizarInterfaceComNovaChamada(registro);
    }
  });
}
