...
firebase.database().ref('ultimaAcao').on('value', snapshot => {
  ...
  if (acao.clientId === clientId) return;  // evitar loop

  if (acao.tipo === 'chamarSenha') {
    chamarSenhaSincronizada(acao.dados.tipoSenha, acao.dados.guiche, true);
...
async function chamarSenhaSincronizada(tipo, guiche, remoto = false) {
  if (!remoto) {
