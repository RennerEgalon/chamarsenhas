...
function atualizarInterfaceComNovaChamada(registro) {
  if (registro.tipo === "Senha Normal" || registro.tipo === "Senha Preferencial") {
    ...
    // Extrair apenas o número da senha: exemplo "Senha 1 - Guichê 1" --> "Senha 1"
    const numeroSenha = registro.senha.split(" - ")[0]; 

    // Iterar sobre ambos os guichês
    ['guiche1', 'guiche2'].forEach(guiche => {
      ...
      if (coluna) {
        ...
        botoes.forEach(btn => {
          // Verifica se o texto começa com o mesmo número de senha
          if (btn.textContent.startsWith(numeroSenha)) {
            const classeDestaque = isPreferencial ? 'botao-destacado-preferencial' : 'botao-destacado-normal';
            btn.classList.add(classeDestaque);
    ...
    // Atualizar o quadro com a última senha chamada
    if (isPreferencial) {
