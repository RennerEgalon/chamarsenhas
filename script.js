...
let ultimaAcaoExecutada = 0;


function enviarAcaoFirebase(tipo, dados) {
  const acao = {
    tipo,
    dados,
    timestamp: Date.now()
  };
  firebase.database().ref('ultimaAcao').set(acao);
}

firebase.database().ref('ultimaAcao').on('value', snapshot => {
  const acao = snapshot.val();
  if (!acao) return;

  // Evitar loop infinito: se a ação já foi feita localmente, ignore.
  if (acao.timestamp <= ultimaAcaoExecutada) return;

  ultimaAcaoExecutada = acao.timestamp;

  if (acao.tipo === 'chamarSenha') {
    chamarSenhaSincronizada(acao.dados.tipoSenha, acao.dados.guiche);
  } else if (acao.tipo === 'limparSenhas') {
    limparSenhas();
  } else if (acao.tipo === 'chamadaPorNome') {
    chamarNomeRemoto(acao.dados.nome, acao.dados.guiche);
  }
});
...
async function limparSenhas() {
  ...
  enviarAcaoFirebase('limparSenhas', {});
...
function chamarNome(guiche) {
  ...
  enviarAcaoFirebase('chamadaPorNome', { nome: nome, guiche: guiche });

}

function chamarNomeRemoto(nome, guiche) {
  const mensagem = `Atenção, ${nome}, dirija-se ao guichê ${guiche}`;
  falar(mensagem);
  // Pode adicionar ao histórico se quiser
}
...
async function chamarSenhaSincronizada(tipo, guiche) {
  ...
  enviarAcaoFirebase('chamarSenha', { tipoSenha: tipo, guiche: guiche });
