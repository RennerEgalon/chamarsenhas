...
function destacarSenhaEmTodosGuiches(tipo, numeroSenha) {
  // tipo: 'normal' ou 'preferencial'
  // numeroSenha: número da senha a destacar (ex: 1, 2, 3...)

  // Colunas para destacar, para normal e preferencial
  const colunasNormais = ['coluna-normal-guiche1', 'coluna-normal-guiche2'];
  const colunasPreferenciais = ['coluna-preferencial-guiche1', 'coluna-preferencial-guiche2'];

  const colunas = tipo === 'normal' ? colunasNormais : colunasPreferenciais;

  // Primeiro, remover o destaque atual
  ...
  // Agora, adicionar destaque apenas nos botões da senha exata
  const classeDestaque = tipo === 'normal' ? 'botao-destacado-normal' : 'botao-destacado-preferencial';
...
function sinalizarLimpezaSenhas() {
  firebase.database().ref('limparSenhas').set({
    timestamp: Date.now()
  });
}

firebase.database().ref('limparSenhas').on('value', async (snapshot) => {
  const data = snapshot.val();
  if (data) {
    console.log('⚠️ Recebido sinal para limpar senhas');
    await limparSenhas();
  }
});


firebase.database().ref('contadorNormal').on('value', (snapshot) => {
...
firebase.database().ref('contadorPreferencial').on('value', (snapshot) => {
  const valor = snapshot.val();
  if (valor) {
      ...
      botoes.forEach(btn => {
        ...
        if (match) {
          ...
          if (num <= limite) {
            ...
            btn.classList.remove('botao-destacado-preferencial', 'botao-destacado-normal');
          }
        }
      });
    }
...
firebase.database().ref('historicoChamadas').on('child_added', (snapshot) => {
  const novaChamada = snapshot.val();
  // Adicione no seu array local e atualize a interface, se necessário
  historicoChamadas.push(novaChamada);
});
...
async function chamarSenhaSincronizada(tipo, guiche) {
  ...
  if (botao) {
  botao.click();
  // Depois do click (que já chama a fala, etc), faça:
  const numeroSenha = contador; // número da senha chamada
  destacarSenhaEmTodosGuiches(tipo, numeroSenha);
} else {
